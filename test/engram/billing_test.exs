defmodule Engram.BillingTest do
  ...
  describe "tier_from_subscription/1" do
    ...
    test "logs paddle_unknown_price_id once per unknown price per process" do
      ...
      log =
        ExUnit.CaptureLog.capture_log(fn ->
          ...
          assert "starter" == Billing.tier_from_subscription(paddle_sub_unknown.("pri_unknown_a"))
          assert "starter" == Billing.tier_from_subscription(paddle_sub_unknown.("pri_unknown_a"))
          assert "starter" == Billing.tier_from_subscription(paddle_sub_unknown.("pri_unknown_a"))

          # A different unknown price → should also log once.
          assert "starter" == Billing.tier_from_subscription(paddle_sub_unknown.("pri_unknown_b"))
        end)
    ...
    test "does NOT log for known price IDs" do
      starter_id = Application.get_env(:engram, :paddle_starter_price_id)
      pro_id = Application.get_env(:engram, :paddle_pro_price_id)
