          ...
          return (
            <li key={s.id} className="flex items-center justify-between gap-2 text-sm">
              <span className="text-foreground">
                {name}
                {isCurrent && (
                  <span className="ml-2 rounded bg-muted px-1.5 py-0.5 text-xs text-muted-foreground">Current</span>
                )}
              </span>
              {!isCurrent && (
                <Button variant="ghost" size="sm" aria-label={`Revoke ${name}`} onClick={() => revoke(s)}>
                  Revoke
                </Button>
              )}
            </li>
