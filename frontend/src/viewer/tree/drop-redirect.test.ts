...
describe("resolveDropMove", () => {
	it("moves a note into a folder destination", () => {
		expect(resolveDropMove([{ id: "n:1", parentId: "root" }], "f:projects")).toEqual({
			dest: "f:projects",
			ids: ["n:1"],
		});
	});
	...
	it("is a no-op when every source is already in the destination (same-folder drop)", () => {
		expect(resolveDropMove([{ id: "n:1", parentId: "f:projects" }], "f:projects")).toBeNull();
	});
	...
	it("is a no-op when the destination is undefined", () => {
		expect(resolveDropMove([{ id: "n:1", parentId: "f:a" }], undefined)).toBeNull();
	});
});
