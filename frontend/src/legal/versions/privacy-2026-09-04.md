...
This Privacy Policy covers the personal data we process across Engram: both the marketing site at `engram.page` and the application at `app.engram.page`, including your account data and the notes and attachments you sync.
...
**The waitlist is closed.** Engram is open for sign-up, so this site no longer collects email addresses. Account creation happens at `app.engram.page` and is covered by the sections below.

We still hold the email addresses of people who joined the waitlist while it was open, and we will use them for the launch updates they signed up to receive. You can unsubscribe from any of those emails, and we will delete your address within 30 days of doing so. While the waitlist was open we also derived a SHA-256 hash of each address for use as a pseudonymous identifier in our analytics; we no longer generate these, though hashes created before the waitlist closed may persist in PostHog for the retention period below.

If you arrived through a referral link, we may remember the source that brought you here (see the Referral section below).
...
| Retaining waitlist email addresses and sending the launch updates they were collected for | Consent (Art. 6(1)(a)), withdrawable at any time by unsubscribing |
...
| [Cloudflare](https://www.cloudflare.com/privacypolicy/) | Marketing site hosting (Workers), CDN | IP address, user agent, request data | Global edge | DPF (EU/UK/Swiss) + SCCs |
...
| [PostHog](https://posthog.com/privacy) | Cookieless product + marketing analytics | Anonymous in-memory session events; pseudonymous hash-based identifiers generated before the waitlist closed | US | DPF (EU) + SCCs |
...
| [Grafana Cloud](https://grafana.com/legal/privacy-policy/) (Grafana Labs) | Application log and metric aggregation (Loki / Prometheus) for operations and debugging | Operational logs and metrics (may include user ID, IP address, and request metadata) | US / EU | SCCs |
...
- **Waitlist email:** no longer collected. Addresses gathered while the waitlist was open are retained until you unsubscribe, then deleted within 30 days.
